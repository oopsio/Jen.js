  ...
  return (
    <Fragment>
      ...
      <p style={styles.subtitle}>
        A VitePress competitor - Markdown-first documentation SSG built for
        Jen.js
      </p>
...
# http://localhost:5173/docs/my-page.md`}
      ...
      <section style={styles.section}>
        ...
        <ul style={styles.list}>
          ...
          <li>
            <a href="/docs/guide/getting-started.md" style={styles.link}>
              Getting Started
            </a>
          </li>
          <li>
            <a href="/docs/api/overview.md" style={styles.link}>
              API Overview
            </a>
          </li>
        </ul>
...
function Breadcrumbs({ path }: { path: string }) {
  ...
  if (lastPart) {
    breadcrumbs.push({
      label:
        lastPart.replace(".md", "").charAt(0).toUpperCase() +
        lastPart.slice(1).replace(".md", ""),
    });
  }
      ...
      <div style={styles.contentWrapper}>
        <div style={{ flex: 1 }}>
          ...
          <div style={styles.editLink}>
            <a
              href={`https://github.com/kessud2021/Jen.js/edit/main/packages/jenpress/${path}`}
              style={styles.link}
              target="_blank"
...
async function renderMarkdown(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (calloutMatch) {
      ...
      output.push(
        `<div class="callout callout-${type}"><div class="callout-header"><span class="callout-icon">${icon}</span><span class="callout-title">${title || type.toUpperCase()}</span></div><div class="callout-content">${formatInline(calloutContent)}</div></div>`,
      );
      continue;
    }
...
const DEFAULT_SIDEBAR: SidebarItem[] = [
  ...
  {
    text: "API Reference",
    items: [{ text: "Overview", link: "docs/api/overview.md" }],
  },
];
    ...
    <div class={`app ${isDarkMode ? "dark" : "light"}`} style={styles.app}>
      <nav
        style={{
          ...
          borderColor: isDarkMode ? "#333333" : "#e1e4e8",
        }}
      >
        <div style={styles.navBrand}>
          <a
            href="#"
            style={{ ...styles.brandLink, color: "#0070f3" }}
            onClick={(e) => {
              e.preventDefault();
        ...
        <ul style={styles.navMenu}>
          <li>
            <a
              href="#docs/index.md"
              style={{ ...styles.link, color: "#0070f3" }}
            >
              Docs
            </a>
          </li>
          <li>
            <a
              href="#docs/guide/getting-started.md"
              style={{ ...styles.link, color: "#0070f3" }}
            >
              Guide
            </a>
