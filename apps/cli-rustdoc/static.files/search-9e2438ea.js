...
const initSearch = async function (Stringdex, RoaringBitmap, hooks) {
  ...
  class DocSearch {
    ...
    async getTypeNameIdsAsync(nn) {
      ...
      const typeNameIdOfOutput = await first(
        output,
        itemTypes.associatedtype,
        "",
      );
      const typeNameIdOfFnPtr = await first(fn, itemTypes.primitive, "");
      const typeNameIdOfFn = await first(fn, itemTypes.trait, "core::ops");
      const typeNameIdOfFnMut = await first(
        fnMut,
        itemTypes.trait,
        "core::ops",
      );
      const typeNameIdOfFnOnce = await first(
        fnOnce,
        itemTypes.trait,
        "core::ops",
      );
      ...
      const typeNameIdOfReference = await first(
        reference,
        itemTypes.primitive,
        "",
      );
    ...
    async execQuery(parsedQuery, filterCrates, currentCrate) {
      ...
      const innerRunNameQuery = async function* (currentCrate) {
        ...
        const handleAlias = async (name, alias, dist, index) => {
          ...
          const path_dist =
            name.includes(" ") || parsedQuery.elems.length === 0
              ? 0
              : checkRowPath(parsedQuery.elems[0].pathWithoutLast, item);
          if (path_dist === null) {
            return null;
          }
          return {
