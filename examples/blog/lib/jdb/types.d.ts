 ...
 */
export type FilterOperator = "$eq" /** Equal to the specified value. */ | "$ne" /** Not equal to the specified value. */ | "$gt" /** Greater than the specified value. */ | "$gte" /** Greater than or equal to the specified value. */ | "$lt" /** Less than the specified value. */ | "$lte" /** Less than or equal to the specified value. */ | "$in" /** Value is in the specified array. */ | "$nin" /** Value is not in the specified array. */ | "$regex"; /** Value matches the specified regular expression pattern. */
/**
 ...
 */
export type UpdateOperator = "$set" /** Set field value(s). Overwrites existing values. */ | "$unset" /** Remove field(s) from the document. */ | "$inc" /** Increment numeric field(s) by the specified amount. */ | "$push" /** Append value(s) to an array field. Creates array if it doesn't exist. */ | "$pull"; /** Remove all occurrences of value(s) from an array field. */
/**
...
export interface ICollection<T extends Document = Document> {
     ...
     */
    insert(doc: Omit<T, "_id" | "_created" | "_updated"> & {
        _id?: string;
    }): Promise<T>;
    /**
     ...
     */
    insertMany(docs: (Omit<T, "_id" | "_created" | "_updated"> & {
        _id?: string;
    })[]): Promise<T[]>;
    /**
