...
import { getAdvancedRouteConfig, validateQueryParams, processQueryParams, } from "./advanced.js";
import { getRedirect, getNotFoundHandler, send404, sendRedirect, createDefault404Html, } from "./handlers.js";
...
export class AdvancedRouter {
    ...
    async resolve(pathname, url, headers, cookies) {
        ...
        if (advancedConfig.redirect) {
            ...
            const target = typeof advancedConfig.redirect.to === "function"
        ...
        if (advancedConfig.guards && advancedConfig.guards.length > 0) {
            ...
            for (const guard of advancedConfig.guards) {
                ...
                if (result !== true) {
                    ...
                    log.info(`[Router] Guard blocked: ${pathname} (status: ${result.status})`);
                    return {
                        type: "redirect",
