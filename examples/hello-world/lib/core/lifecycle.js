...
export class GracefulShutdown {
        ...
        return new Promise((resolve) => {
            const poll = () => {
                if (this.activeRequests.size === 0) {
                    log.info(`[Graceful Shutdown] All ${this.activeRequests.size} requests completed`);
                    resolve();
                    return;
                }
                const elapsed = Date.now() - startTime;
                if (elapsed > this.shutdownTimeout) {
                    log.warn(`[Graceful Shutdown] Timeout after ${elapsed}ms with ${this.activeRequests.size} requests still active`);
                    resolve();
                    return;
                }
    ...
    async handleShutdownSignal(signal, onClose) {
        ...
        log.info(`[Graceful Shutdown] ${this.activeRequests.size} active request(s)`);
