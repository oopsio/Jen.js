...
async function main() {
  ...
  // Clean up any heavy polyfills generated by the initial build step before hashing
  const rawDistFiles = getAllFiles(distDir, [".js"]);
  for (const file of rawDistFiles) {
    if (basename(file).includes("polyfill")) {
      await fs.unlink(file).catch(() => {});
      console.log(`[CLEANUP] Deleted heavy polyfill file: ${basename(file)}`);
    }
  }
  ...
  // Step 1: Hash assets in parallel (Polyfills are already deleted)
  ...
  await Promise.all(htmlFiles.map(async (filePath) => {
    ...
    // 1.5. Aggressively strip out any polyfill script tags from the HTML
    content = content.replace(/<script\s+[^>]*src=["']?[^>]*polyfills?[^>]*["']?[^>]*><\/script>/gi, "");
