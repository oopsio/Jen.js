...
export async function buildSite(opts) {
    ...
    if (config.features?.compilers !== false) {
        ...
        const componentFiles = readdirSync(siteSourceDir, { recursive: true }).filter((f) => /\.(vue|svelte)$/.test(String(f)));
    ...
    writeFileSync(preactBundleEntry, `export * from 'preact'; export * from 'preact/hooks'; export * from 'preact/compat';
