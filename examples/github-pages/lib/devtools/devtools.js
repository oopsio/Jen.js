...
import { ComponentTreeManager } from "./component-tree.js";
...
export class DevTools {
    ...
    setupDragging() {
        ...
        header.addEventListener("mousedown", ((e) => {
            const me = e;
            if (me.target.closest(".jen-devtools-controls"))
                return;
            isDragging = true;
    ...
    setupTreeNavigation() {
        ...
        tree.addEventListener("click", (e) => {
            const node = e.target.closest(".jen-tree-node");
            if (node) {
    ...
    setupSearch() {
        if (!this.panel)
            return;
        const searchInput = this.panel.querySelector(".jen-devtools-search");
        if (!searchInput)
            return;
    ...
    selectTab(tabName) {
        ...
        contents.forEach((content) => {
            ...
            contentEl.classList.toggle("active", contentEl.dataset.tab === tabName);
        });
    ...
    renderInspector() {
        if (!this.panel || !this.selectedComponent)
            return;
        const inspector = this.panel.querySelector(".jen-devtools-inspector");
        if (!inspector)
            return;
    ...
    renderComponentTree() {
        if (!this.panel)
            return;
        const treeContainer = this.panel.querySelector(".jen-devtools-tree");
        if (!treeContainer)
            return;
    ...
    renderSearchResults(results) {
        if (!this.panel)
            return;
        const resultsContainer = this.panel.querySelector(".jen-search-results");
        if (!resultsContainer)
            return;
     ...
     */
    registerComponent(id, name, el, props, state, hooks) {
     ...
     * Log an event
     */
    logEvent(componentId, eventName, data) {
