...
export async function buildSite(opts = {}) {
    ...
    try {
        ...
        log.info(`[build-site] Build complete in ${duration}ms (${bytesOriginal} → ${bytesOptimized} bytes)`);
        return buildStats;
    }
    catch (error) {
        log.error(`[build-site] Build failed: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
/**
