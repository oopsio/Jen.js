...
import { scanLayouts, buildLayoutHierarchy, resolveLayoutStack, renderWithLayoutStack, collectLayoutHeads, } from "../core/layouts/index.js";
...
export async function renderRouteToHtml(opts) {
    ...
    for (const headNode of layoutHeads) {
        try {
            ...
            console.error(`Failed to render Head component:`, err instanceof Error ? err.message : String(err));
        }
    }
...
<div id="app">${bodyHtml}</div>`;
    ...
    if (shouldHydrate) {
        ...
        if (frameworkDataStr.length > MAX_DATA_SIZE) {
            throw new Error(`Framework data exceeds maximum size of ${MAX_DATA_SIZE} bytes. ` +
