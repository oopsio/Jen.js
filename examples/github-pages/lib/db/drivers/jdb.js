import { JDBEngine } from "../../jdb/index.js";
...
export class JDBDriver {
    ...
    constructor(config) {
        try {
            ...
            log.info(`Initializing JDB driver: ${jdbConfig.inMemory ? "in-memory" : `file-based (${jdbConfig.root})`}`);
            this.engine = new JDBEngine(jdbConfig);
        }
        catch (error) {
            log.error(`JDB driver initialization failed: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    /**
    ...
    async connect() {
        try {
            ...
            log.error(`JDB connection failed: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    /**
    ...
    async disconnect() {
        try {
            ...
            log.error(`JDB disconnection failed: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    /**
    ...
    async create(collection, data) {
        try {
            ...
            const result = await this.engine.collection(collection).insert(data);
    ...
    async delete(collection, filter) {
        try {
            if (!filter || Object.keys(filter).length === 0) {
                throw new Error("Delete requires at least one filter condition. Prevent full collection deletion.");
            }
