...
import { info, success, error as logError, formatSize } from '../../utils/log.js';
...
export async function analyze(entryArg: string | undefined, options: AnalyzeOptions = {}): Promise<void> {
  try {
    ...
    try {
      ...
      const totalSize = modules.reduce((sum, m) => sum + Buffer.byteLength(m.source, 'utf8'), 0);
      ...
      // Print module sizes
      console.log('\n📈 Module Sizes\n');
      modules.sort((a, b) => Buffer.byteLength(b.source, 'utf8') - Buffer.byteLength(a.source, 'utf8')).slice(0, 10).forEach((m) => {
...
function printModuleTree(module: any, graph: any, visited: Set<string>, depth: number): void {
