    ...
    if (typeof cond === 'object' && cond !== null && !Array.isArray(cond)) {
      for (const op in cond) {
        ...
        switch (op) {
          ...
          case '$in': if (!target.includes(val)) return false; break;
          case '$nin': if (target.includes(val)) return false; break;
          case '$regex': if (!new RegExp(target).test(val)) return false; break;
        }
      }
    } else {
...
export function applyUpdate(doc: any, update: any) {
  ...
  for (const op in update) {
    ...
    for (const key in fields) {
      ...
      switch (op) {
        case '$set': doc[key] = val; break;
        case '$unset': delete doc[key]; break;
        case '$inc': doc[key] = (doc[key] || 0) + val; break;
        case '$push': 
