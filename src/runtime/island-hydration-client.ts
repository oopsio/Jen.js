...
/**
 * Client-side island hydration orchestrator.
 * Discovers interactive islands from server-rendered HTML and hydrates them
 * based on their configured strategy (eager, idle, or viewport-based).
 * This enables efficient partial hydration where only interactive components
 * receive JavaScript, keeping the rest of the page as static HTML.
 */

/**
 * Metadata for an interactive island discovered in the page.
 */
interface Island {
  ...
  component: string;
  /** Hydration timing: "load" (immediate), "idle" (deferred), "visible" (intersection). */
