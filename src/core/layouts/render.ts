...
import type { LayoutEntry } from "./scan.js";
import type { LayoutModule, ResolvedLayoutStack } from "./types.js";
 ...
 */
async function loadLayoutModule(filePath: string): Promise<LayoutModule> {
 ...
 */
export async function resolveLayoutStack(
  layoutEntries: LayoutEntry[],
): Promise<ResolvedLayoutStack> {
...
export function collectLayoutHeads(
  ...
  for (const layoutMod of layoutStack.modules) {
    if (layoutMod.Head) {
      const headNode = h(layoutMod.Head as any, props);
      heads.push(headNode);
    }
  }
  ...
  if (pageHeadComponent) {
    const headNode = h(pageHeadComponent as any, props);
    heads.push(headNode);
  }
