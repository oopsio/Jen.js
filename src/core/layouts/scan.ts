...
/**
 * Represents a discovered layout file with filesystem metadata.
 * Layouts are named (layout).tsx and are discovered in the directory hierarchy.
 */
export type LayoutEntry = {
  /**
   * Unique identifier for the layout, derived from its directory path.
   * Example: "src_pages_blog" for "src/pages/blog/(layout).tsx"
   */
  id: string;

  /**
   * Absolute filesystem path to the layout file.
   * Example: "/app/src/pages/blog/(layout).tsx"
   */
  filePath: string;

  /**
   * Directory depth relative to siteDir.
   * Used to determine hierarchy (0 = root layout, 1 = first level, etc.)
   */
  depth: number;

  /**
   * Directory path relative to siteDir where this layout is located.
   * Example: "pages/blog" for a layout at "src/pages/blog/(layout).tsx"
   */
  dirPath: string;
};
 ...
 */
export function scanLayouts(config: FrameworkConfig): LayoutEntry[] {
