...
/**
 * Route metadata with rendering information
 */
...
/**
 * Vercel Build Output API config.json structure
 */
...
/**
 * ISR prerender config structure
 */
 ...
 */
export function detectRevalidateSeconds(filePath: string): number | undefined {
 ...
 */
export function functionNameFromRoute(route: RouteEntry): string {
  // /posts/:id => posts_id
  // /api/users => api_users
 ...
 */
export function convertRoutePattern(route: RouteEntry): string {
...
export class VercelOutputBuilder {
  ...
  private ssrRoutes: RouteMetadata[] = [];
  private ssgRoutes: RouteMetadata[] = [];
  private isrRoutes: RouteMetadata[] = [];

  constructor(outputDir: string, config: FrameworkConfig) {
   ...
   */
  private async generateSsgStatics() {
   ...
   */
  private generateHandlerCode(route: RouteEntry): string {
...
function extractParams(req, pattern) {
   ...
   */
  private generatePrerenderConfig() {
...
export async function buildVercelOutput(
  ...
  for (const route of routes) {
    const mode = detectRenderingMode(route.filePath) as "ssg" | "ssr" | "isr";
