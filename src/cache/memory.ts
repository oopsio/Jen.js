...
export class MemoryCache {
  ...
  set(key: string, value: any, ttlMs?: number): void {
    try {
      ...
      log.error(
        `Cache set failed for ${key}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  get<T = any>(key: string): T | undefined {
    try {
      ...
      log.error(
        `Cache get failed for ${key}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  delete(key: string): void {
    try {
      ...
      log.error(
        `Cache delete failed for ${key}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
   ...
   */
  stats(): { total: number; withTTL: number; keys: string[] } {
    let withTTL = 0;
    for (const entry of this.store.values()) {
      if (entry.expiresAt) withTTL++;
    }
