...
export class ComponentTreeManager {
  ...
  getNextComponent(current: ComponentNode): ComponentNode | null {
    if (current.children && current.children.length > 0) {
      return current.children[0];
    }

    let node: ComponentNode | null = current;
    while (node) {
      if (node.parent) {
  ...
  getPreviousComponent(current: ComponentNode): ComponentNode | null {
    if (current.parent) {
  ...
  private getLastDescendant(node: ComponentNode): ComponentNode {
